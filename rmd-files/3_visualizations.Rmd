...
The geom_bar() function can be used to create a graph. To try this, first fetch a bit more data from OBIS:
...
Binning the data by year doesnt always work, so to give a better display, we will create a new column using dplyr mutate(), and do a bit of math to create 10 year bins for the data.

```{r dor}
library(dplyr)

binned_dor <- dor %>% mutate(yearBins=round(as.integer(year)/10)*10 )

ggplot() +
 geom_bar(data = binned_dor,stat='count', aes(x = yearBins), binwidth = 2)
...
```{r binned_dor}
ggplot() +
 geom_histogram(data = binned_dor, stat="count", aes(x = yearBins, fill = family), binwidth = 2) +
